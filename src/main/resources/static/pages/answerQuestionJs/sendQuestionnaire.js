...
document.getElementById(
    "ctl02_ContentPlaceHolder1_InviteEmail1_hrefSend").innerText = "批量发送问卷 — "
    + nameOfQuestionnaire;
...
if (getCookie("changeTableType") == "shortMessageSend") {   //从已发问卷页面点击进入的问卷发布页面
  ...
  document.getElementById('linkSendA').removeAttribute("class",
      "inside-wrapper nav-show nav-items clicked linkCode-icon");
  document.getElementById('linkSendA').setAttribute("class",
      "inside-wrapper nav-show nav-items linkCode-icon");

  document.getElementById(
      'shortMessageSendA').style.backgroundColor = "#1ea0fa";
  document.getElementById('shortMessageSendA').removeAttribute("class",
      "inside-wrapper nav-show nav-items sms-icon");
  document.getElementById('shortMessageSendA').setAttribute("class",
      "inside-wrapper nav-show nav-items clicked sms-icon");
...
function send(value) {
  if (value == 0) {
    ...
    document.getElementById(
        'ctl02_ContentPlaceHolder1_btnSend').style.display = '';
  } else if (value == 1) {
    sendType = '1';
    //短信内容隐藏
    ...
    document.getElementById(
        'ctl02_ContentPlaceHolder1_btnSend').style.display = 'none';
  ...
  fileReader.onload = function (ev) {
    ...
    if (persons.length != 0) {
      ...
      for (var i = 0; i < persons.length; i++) {
        _$("#userInfoTable").bootstrapTable('insertRow',
            {index: i, row: persons[i]});
...
function layOutSend() {
  ...
  if (sendType == '0') {
    ...
    //邮件发送富文本内容
    var emailContent = document.getElementById(
        "ctl02_ContentPlaceHolder1_fckEmailContent");
    ...
    } else if (emailContent.search("【联系人姓名】") == -1 || emailContent.search(
        "【填写问卷地址】") == -1) {
...
function getQrcode() {
  ...
  _$.ajax({
    ...
    success: function (res) {
      ...
      document.getElementById(
          'ctl02_ContentPlaceHolder1_txtLink').value = shortUrl;
      // $('#code').qrcode(); //任意字符串
